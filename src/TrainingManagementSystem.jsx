...
import { Check, ChevronDown, ChevronRight, Link as LinkIcon, Trash2, Edit, Plus, X, LogOut, Users, TrendingUp, BookOpen, Menu } from 'lucide-react';
              ...
              <div className="space-y-2">
                {trainingPaths.map(path => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 rounded-lg transition-colors ${selectedPath === path.id ? 'bg-indigo-100 text-indigo-700 font-medium' : 'hover:bg-gray-100 text-gray-700'}`}
                  >
                    {path.name}
                  </button>
                ))}
                    ...
                    <div key={category.id} className="bg-white rounded-lg shadow-sm overflow-hidden">
                      ...
                      >
                        ...
                        <button
                          ...
                          className={`flex items-center space-x-2 px-4 py-2 rounded-lg transition-colors ${isCompleted ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                        >
                        ...
                        <div className="border-t border-gray-200 p-4 bg-gray-50 space-y-4">
                          {category.documentation && (
                            <div>
                              ...
                              <a href={category.documentation} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-2 text-indigo-600 hover:text-indigo-700">
                          ...
                          {category.video && (
                            <div>
                              ...
                              <a href={category.video} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-2 text-indigo-600 hover:text-indigo-700">
                                <LinkIcon className="w-4 h-4" />
                                <span className="text-sm">Watch Video</span>
                              </a>
                            </div>
                          )}
                          {!category.documentation && !category.video && <p className="text-sm text-gray-500 italic">No materials available yet</p>}
                        </div>
                      )}
                    </div>
                  );
      ...
      <div className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              <button onClick={() => onChangeView('training')} className="px-4 py-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">My Training</button>
              <button onClick={onLogout} className="flex items-center space-x-2 px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-lg transition-colors">
                <LogOut className="w-4 h-4" />
                <span>Logout</span>
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div><p className="text-sm text-gray-600">Total Staff</p><p className="text-3xl font-bold text-gray-800">{branchStaff.length}</p></div>
              <Users className="w-12 h-12 text-indigo-600" />
            </div>
          </div>
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div><p className="text-sm text-gray-600">Branch Progress</p><p className="text-3xl font-bold text-indigo-600">{branchProg.percentage}%</p></div>
              <TrendingUp className="w-12 h-12 text-green-600" />
            </div>
          </div>
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div><p className="text-sm text-gray-600">Total Modules</p><p className="text-3xl font-bold text-gray-800">{trainingPaths.reduce((sum, path) => sum + path.categories.length, 0)}</p></div>
              <BookOpen className="w-12 h-12 text-purple-600" />
            </div>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200"><h2 className="text-lg font-semibold text-gray-800">Staff Progress</h2></div>
                  ...
                  return (
                    <tr key={staff.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center"><div className="w-full bg-gray-200 rounded-full h-2 mr-2"><div className="bg-indigo-600 h-2 rounded-full" style={{ width: `${prog.percentage}%` }}></div></div><span className="text-sm font-medium text-gray-700">{prog.percentage}%</span></div>
                      </td>
                    </tr>
                  );
                })}
...
const AdminDashboard = ({ currentUser, users, branches, trainingPaths, progress, onLogout, onRenamePath, onAddCategory, onUpdateCategory, onDeleteCategory, onRenameCategory }) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6 border-b border-gray-200">
          <div className="flex space-x-8">
            <button onClick={() => setActiveTab('overview')} className={`pb-4 px-2 font-medium transition-colors ${activeTab === 'overview' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>Overview</button>
            <button onClick={() => setActiveTab('manage')} className={`pb-4 px-2 font-medium transition-colors ${activeTab === 'manage' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>Manage Training</button>
          </div>
        </div>
        ...
        {activeTab === 'overview' ? (
          <>
                ...
                return (
                  <div key={branch.id} className="bg-white rounded-lg shadow-sm p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div><h3 className="text-lg font-semibold text-gray-800">{branch.name}</h3><p className="text-sm text-gray-600">Manager: {manager?.name}</p></div>
                      <div className="text-right"><div className="text-2xl font-bold text-indigo-600">{branchProg.percentage}%</div><div className="text-sm text-gray-600">{branchProg.staffCount} staff</div></div>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2"><div className="bg-indigo-600 h-2 rounded-full" style={{ width: `${branchProg.percentage}%` }}></div></div>
            ...
            <div className="bg-white rounded-lg shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-200"><h2 className="text-lg font-semibold text-gray-800">All Staff Progress</h2></div>
              <div className="overflow-x-auto">
                <table className="w-full">
                ...
                <div className="bg-indigo-50 px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                  {editingPath === path.id ? (
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <button onClick={() => setEditingPath(path.id)} className="p-1 hover:bg-indigo-100 rounded"><Edit className="w-4 h-4 text-indigo-600" /></button>
                    </div>
                  )}
                  <button onClick={() => setShowAddCategory(path.id)} className="flex items-center space-x-2 px-3 py-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm"><Plus className="w-4 h-4" /><span>Add Category</span></button>
                </div>

                {showAddCategory === path.id && (
                  <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
                    <div className="flex items-center space-x-2">
                      <input type="text" value={newCategoryName} onChange={(e) => setNewCategoryName(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && newCategoryName && onAddCategory(path.id, newCategoryName, () => { setShowAddCategory(null); setNewCategoryName(''); })} placeholder="Category name" className="flex-1 px-3 py-2 border border-gray-300 rounded-lg" autoFocus />
                      <button onClick={() => newCategoryName && onAddCategory(path.id, newCategoryName, () => { setShowAddCategory(null); setNewCategoryName(''); })} className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Add</button>
                      <button onClick={() => { setShowAddCategory(null); setNewCategoryName(''); }} className="p-2 text-gray-600 hover:bg-gray-200 rounded-lg"><X className="w-5 h-5" /></button>
                    </div>
                  </div>
                )}
                    ...
                    <div key={category.id} className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        {editingCategory === category.id ? (
                          ...
                          <div className="flex items-center space-x-2">
                            <h3 className="font-medium text-gray-800">{category.name}</h3>
                            <button onClick={() => setEditingCategory(category.id)} className="p-1 hover:bg-gray-100 rounded"><Edit className="w-3 h-3 text-gray-600" /></button>
                          </div>
                        )}
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <input type="text" value={category.documentation || ''} onChange={(e) => onUpdateCategory(path.id, category.id, 'documentation', e.target.value)} placeholder="https://example.com/documentation.pdf" className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="flex items-center space-x-2">
                            <LinkIcon className="w-4 h-4 text-gray-400" />
                            <input type="text" value={category.video || ''} onChange={(e) => onUpdateCategory(path.id, category.id, 'video', e.target.value)} placeholder="https://youtube.com/watch?v=..." className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                          </div>
                        </div>
                      </div>
                    </div>
...
const TrainingManagementSystem = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const { data: pathsData } = await supabase.from('training_paths').select(`*, categories(*)`).order('id');
      if (pathsData) {
  ...
  const addCategory = async (pathId, categoryName, onSuccess) => {
    ...
    const newCategory = { id: newId, path_id: pathId, name: categoryName, documentation: '', video: '' };
